	...
	else if(oCiC) //circle into circle situation.
	{
		...
		for(int j = 0; j < y1; ++j){
			for(int i = 0 ; i < baseCircleCounter ; ++i)
			{
				...
				myCircleVector.push_back(temp1InnerCircle);
			...
			for(int i = 0 ; i < yCounter ; ++i)
			{
				for(int j = 0 ; j < xCounter ; ++j)
				{
					if(smallWidth < (2 * bigRadius - (tempExtraSpace + incX)))
					{
						...
						myRectangleVector.push_back(tempInnerRectangle1);
...
void ComposedShape::draw(std::ofstream &inputFile) 
{
	...
	if(oRiR)
	{
		...
		inputFile << fileWriter;
		inputFile << '\n';
		for(auto i = 0 ; i < myRectangleVector.size() ; ++i)
		{
			myRectangleVector[i].draw(inputFile); //drawing vector of rectangles.
			inputFile << "\n";
		}
	}
	else if(oRiC)
	{
		...
		+ " width=\"" + to_string(myOuterRectangle.getWidth()) + "\"" + " height=\"" + to_string(myOuterRectangle.getHeight()) + "\"" + " stroke=\"red\"" +
	 	...
	 	inputFile << fileWriter;
		inputFile << '\n';
		for(auto i = 0 ; i < myCircleVector.size() ; ++i)
		{
			myCircleVector[i].draw(inputFile); //drawing vector of circles.
			inputFile << "\n";
		}
	...
	else if(oRiT)
	{
		...
		+ " width=\"" + to_string(myOuterRectangle.getWidth()) + "\"" + " height=\"" + to_string(myOuterRectangle.getHeight()) + "\"" + " stroke=\"red\"" +
	 	...
	 	inputFile << fileWriter;
		inputFile << "\n";
		for(auto i = 0 ; i < myTriangleVector.size() ; ++i)
		{

			myTriangleVector[i].draw(inputFile); //drawing vector of triangles.
			inputFile << "\n";
		}
	...
	else if(oTiT)
	{
		...
		for(auto i = 0 ; i < myTriangleVector.size() ; ++i) //drawing vector of triangles.
		{
			myTriangleVector[i].draw(inputFile);
			inputFile << "\n";
		}
	...
	else if(oTiR)
	{
		...
		inputFile << "\n";
 		...
 		for(auto i = 0 ; i < myRectangleVector.size() ; ++i) //drawing vector of rectangles.
		{
			myRectangleVector[i].draw(inputFile);
			inputFile << "\n";
		}
